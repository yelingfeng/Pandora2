...
class TableScreenshot {
  ...
  static getTableElement(tableRef) {
    return tableRef && tableRef.$refs && tableRef.$refs.innerTable && tableRef.$refs.innerTable.$el
  }
  ...
  static async exportTableScreenshot(options) {
    ...
    try {
      ...
      // 获取表格相关元素
      const tableWrapper = tableElement.querySelector('.el-table__wrapper')
      ...
      // 下载截图
      this.downloadScreenshot(canvas, filename)
