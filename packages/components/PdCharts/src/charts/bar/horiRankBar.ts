...
export const build = (originData: any, chartConfig?: IChartConfigType) => {
  const {
    names = [],
    values = [],
    percentData = [],
    crossArr = [],
    maxPercent = 100
  } = transformRankBarData(originData) || {}
  const maxLabelLength = names.reduce((acc: number, cur: any) => {
    return Math.max(acc, String(cur || '').length)
  }, 0)
  const barCfg = (chartConfig && chartConfig.bar) || {}
  const cfgLabel = typeof (barCfg as any).label === 'object' ? (barCfg as any).label : {}
  ...
  const cfgRank = typeof (barCfg as any).rank === 'object' ? (barCfg as any).rank : {}
  ...
  const rightLabelCfg = typeof (barCfg as any).rightLabel === 'object' ? (barCfg as any).rightLabel : {}
